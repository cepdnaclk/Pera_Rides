...
export default function Map() {
  ...
  const { mapData, isMapDataLoading, isMapDataError } = useSelector((store) => store.map);

  const onRegionChange = (region) => {
    console.log(region);
  };
  const showLocationsOfInterest = () => {
    return mapData.length && mapData?.map((oneMapData) => {
      return (
        <Marker
          key={oneMapData._id}
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {isMapDataLoading && !isMapDataError 
        ? <Text>Loading...</Text> 
        : !isMapDataLoading && isMapDataError 
        ? <Text>Error Loading Map Data. Please Try Again.</Text> 
